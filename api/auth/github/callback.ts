...
function cookies(req: any) {
  return Object.fromEntries(String(req.headers.cookie ?? '').split(';').filter(Boolean).map((part: string) => {
...
export default async function handler(req: any, res: any) {
  try {
    ...
    const expected = cookies(req).aegis_oauth_state;
    if (!code || !state || !expected || Buffer.byteLength(state) !== Buffer.byteLength(expected) || !crypto.timingSafeEqual(Buffer.from(state), Buffer.from(expected))) return res.status(400).send('Invalid OAuth state or callback parameters.');
