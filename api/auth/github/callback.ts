...
import { encryptGithubToken, createSession } from '../../../../server/auth';
import { upsertGithubUser } from '../../../../server/db';
...
function errorResponse(res: any, code: number, message: string) { res.statusCode = code; res.setHeader('Content-Type', 'text/plain; charset=utf-8'); res.end(message); }
export default async function handler(req: any, res: any) {
  try {
    ...
    if (!code || !state || !expected || Buffer.byteLength(state) !== Buffer.byteLength(expected) || !crypto.timingSafeEqual(Buffer.from(state), Buffer.from(expected))) return errorResponse(res, 400, 'Invalid OAuth state or callback parameters. Please restart GitHub sign-in.');
    ...
    res.setHeader('Set-Cookie', [`aegis_session=${encodeURIComponent(session)}; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=604800`, 'aegis_oauth_state=; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=0']); res.statusCode = 302; res.setHeader('Location', '/'); res.end();
  } catch (error) { errorResponse(res, 503, error instanceof Error ? error.message : 'GitHub login failed'); }
}
