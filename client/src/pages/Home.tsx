...
export default function Home() {
  ...
  const submitLocalAuth = async (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    const data = Object.fromEntries(new FormData(event.currentTarget));
    const response = await fetch("/api/auth/local", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...data, action: authMode }) });
  ...
  if (loading) return <div className="min-h-screen grid place-items-center bg-background text-muted-foreground">{ar ? "جارٍ تحميل AegisCore…" : "Loading AegisCore…"}</div>;
  if (!user && !guest) {
    return <div className="min-h-screen grid place-items-center bg-background px-6"><div className="max-w-lg text-center space-y-5"><div className="mx-auto grid h-16 w-16 place-items-center rounded-2xl border bg-card"><ShieldCheck /></div><h1 className="text-4xl font-semibold">AegisCore</h1><p className="text-muted-foreground">{ar ? "فحص أمني حقيقي للمستودعات، واكتشاف الثغرات، ومعالجة مدعومة بالذكاء الاصطناعي، وسجل تدقيق. سجّل الدخول لإنشاء مساحة عمل." : "Real repository security scanning, findings, AI-assisted remediation, and audit history. Sign in securely to create a workspace."}</p><div className="flex flex-wrap justify-center gap-3"><button className="inline-flex items-center rounded-xl bg-primary px-5 py-3 font-medium text-primary-foreground" onClick={startLogin}>{ar ? "تسجيل الدخول عبر GitHub" : "Sign in with GitHub"}</button><button className="inline-flex items-center rounded-xl border px-5 py-3 font-medium hover:bg-accent" onClick={() => toast.info('Google sign-in will be available after the Google OAuth credentials are added to Vercel.')}>{ar ? "المتابعة عبر Google" : "Continue with Google"}</button></div><form onSubmit={submitLocalAuth} className="mt-4 space-y-2 rounded-xl border p-4 text-left"><div className="flex gap-2"><button type="button" onClick={() => setAuthMode("login")} className={`flex-1 rounded-lg px-3 py-2 text-sm ${authMode === "login" ? "bg-primary text-primary-foreground" : "border"}`}>{ar ? "دخول" : "Sign in"}</button><button type="button" onClick={() => setAuthMode("signup")} className={`flex-1 rounded-lg px-3 py-2 text-sm ${authMode === "signup" ? "bg-primary text-primary-foreground" : "border"}`}>{ar ? "إنشاء حساب" : "Create account"}</button></div>{authMode === "signup" ? <input name="name" required placeholder={ar ? "الاسم" : "Name"} className="w-full rounded-lg border bg-background px-3 py-2 text-sm" /> : null}<input name="email" type="email" required placeholder={ar ? "البريد الإلكتروني" : "Email address"} className="w-full rounded-lg border bg-background px-3 py-2 text-sm" /><input name="password" type="password" required minLength={8} placeholder={ar ? "كلمة المرور (8 أحرف على الأقل)" : "Password (8+ characters)"} className="w-full rounded-lg border bg-background px-3 py-2 text-sm" /><button type="submit" className="w-full rounded-lg bg-secondary px-3 py-2 text-sm font-medium">{authMode === "signup" ? (ar ? "إنشاء الحساب" : "Create account") : (ar ? "تسجيل الدخول" : "Sign in")}</button></form><button type="button" onClick={() => setGuest(true)} className="w-full rounded-xl border px-5 py-3 font-medium hover:bg-accent">{ar ? "الدخول كزائر (بدون حساب)" : "Continue as guest"}</button><p className="text-xs text-muted-foreground">{ar ? "يمكن تفعيل مزودين آخرين من إعدادات OAuth." : "More providers can be enabled from the project OAuth settings."}</p><div className="border-t pt-5 text-sm text-muted-foreground">تم التطوير بواسطة <strong className="text-foreground">Odai Alfaiq</strong><br /><a className="underline hover:text-foreground" href="https://github.com/amrodai195-ai" target="_blank" rel="noreferrer">GitHub: amrodai195-ai</a><span className="mx-2">·</span><a className="underline hover:text-foreground" href="tel:+967774144269">+967 774 144 269</a><span className="mx-2">·</span><a className="underline hover:text-foreground" href="https://wa.me/967774144269" target="_blank" rel="noreferrer">WhatsApp</a></div></div></div>;
  }
  ...
  return <div className="min-h-screen bg-background p-6 lg:p-10">
    <div className="mx-auto max-w-7xl space-y-8">
      ...
      <footer className="border-t pt-5 text-center text-sm text-muted-foreground">تم التطوير بواسطة <strong className="text-foreground">Odai Alfaiq</strong> · <a className="underline hover:text-foreground" href="https://github.com/amrodai195-ai" target="_blank" rel="noreferrer">GitHub</a> · <a className="underline hover:text-foreground" href="https://wa.me/967774144269" target="_blank" rel="noreferrer">WhatsApp</a> · <a className="underline hover:text-foreground" href="tel:+967774144269">اتصال</a></footer>
    </div>
  </div>;
}
